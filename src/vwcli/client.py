...
class Client:
    ...
    def start_bw_serve(self) -> None:
        if self.bw_serve_url:
            if self._bw_serve_ping("/status"):
                ...
                return
            print(
                "[info] Configured BW_SERVE_URL is not reachable; "
                "checking bitwarden-cli.service...",
                file=sys.stderr,
            )
    ...
    def _bw_serve_raw_request(
        ...
        if sock_path:
            ...
            req = urllib.request.Request(
                f"{self.bw_serve_url}{path}", data=body, headers=headers, method=method
            )
            try:
    ...
    def bw_serve_try_json(
        ...
        *,
        ...
        for method, path in specs:
            ...
            errors.append(
                f"{method} {path}: success=false message={response.get('message')}"
            )
    ...
    def bw_list_items_by_collection(self, collection_id: str) -> list[dict[str, Any]]:
        if self.bw_serve_url:
            ...
            result = self._bw_serve_list(
                f"/list/object/items?collectionId={encoded}", "items by collection"
            )
            if result is not None:
        ...
        if self.bw_serve_url:
            try:
                data = self.bw_serve_try_json(
                    "create", item_json, [("POST", "/object/item")]
                )
                if isinstance(data, dict):
                    return data
            except VwcliError:
    ...
    def apply_item_updates(
        ...
        if uris:
            login["uris"] = [
                {"uri": url, "match": match}
                for url, match in (parse_uri(u) for u in uris)
            ]
            ...
            if add_uris:
                existing += [
                    {"uri": url, "match": match}
                    for url, match in (parse_uri(u) for u in add_uris)
                ]
            login["uris"] = existing
    ...
    def cmd_create(self, ns: argparse.Namespace) -> None:
        ...
        item_json = self.apply_item_updates(
            item_json, ns.name, ns.username, password, ns.notes or "", ns.uris
        )
    ...
    def cmd_update(self, ns: argparse.Namespace) -> None:
        ...
        if ns.clone:
            ...
            if ns.organization_id:
                clone_payload = self.set_organization_id(
                    clone_payload, ns.organization_id
                )
            if vault_org_id:
    ...
    def cmd_search(self, ns: argparse.Namespace) -> None:
        ...
        try:
            orgs = self.bw_list_organizations()
            org_map = {
                str(o.get("id") or ""): str(o.get("name") or "")
                for o in orgs
                if o.get("id")
            }
        except VwcliError:
        ...
        if ns.uri_filter:
            ...
            items = [
                ...
                for item in items
                if any(
                    uri_lower in (u.get("uri") or "").lower()
        ...
        col_map = {
            str(c.get("id") or ""): str(c.get("name") or "") for c in collections
        }
    ...
    def cmd_attachment_show(self, ns: argparse.Namespace) -> None:
        ...
        if sys.stdout.isatty():
            ...
            for att in attachments:
                table.add_row(
                    att.get("id", ""), att.get("fileName", ""), att.get("size", "")
                )
            Console().print(table)
        else:
    ...
    def cmd_collections_search(self, ns: argparse.Namespace) -> None:
        ...
        if sys.stdout.isatty():
            table = Table(
                title=f"Collections: {len(matches)}", show_lines=False, highlight=True
            )
            table.add_column("ID", style="dim")
            table.add_column("Name", style="bold")
            table.add_column("Organization ID", style="dim")
    ...
    def cmd_collections_add(self, ns: argparse.Namespace) -> None:
        ...
        if parent_id:
            ...
            parent_data = (
                parent_resp.get("data", {}) if isinstance(parent_resp, dict) else {}
            )
            parent_org = (
                str(parent_data.get("organizationId") or "")
    ...
    def cmd_collections_move(self, ns: argparse.Namespace) -> None:
        ...
        if not items:
            ...
            moved: list[
                tuple[str, str]
            ] = []  # (item_id, item_name) of successfully moved items
            errors = 0
            for item in items:
                item_id = str(item.get("id") or "")
